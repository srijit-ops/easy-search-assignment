...
function TaskCard({ status, name, date, time, id }) {
  ...
  return (
    <div
      className={`rounded-lg p-4 mb-4 w-full bg-white ${
        status === false &&
        " hover:shadow-md hover:border-gray-200 shadow border border-white"
      }`}
    >
      <p
        className={`${
          status === false
            ? "text-[#F4511E] bg-red-200"
            : "text-[#168E14] bg-green-200"
        }  rounded-lg px-4 py-1 w-fit mb-4 text-[0.9rem] font-semibold`}
      >
